// (MODIFIED TO WORK WITH DOMAIN BUILDER)
// NOTE: Because we are assuming that we are running wiht a 64-bit
// hypervisor, we assume that PAE is always availabe so we don't pass
// around extra flags.  Hopefully this is OK, or some fixing will need to
// be done.
  ...
  )
{
    ...
    case 0x80000001: {
        int is_64bit = 1 /*hypervisor_is_64bit(xch)*/ && is_pae;
...
*/
    ...
    if ( strstr(brand, "AMD") )
        amd_xc_cpuid_policy(domid, input, regs, is_pae, is_nestedhvm);
    else
        intel_xc_cpuid_policy(domid, input, regs, is_pae, is_nestedhvm);
...
{
    int xen_64bit = 1/*hypervisor_is_64bit(xch)*/;
    char brand[13];
    uint64_t xfeature_mask;
...
{
    if (is_hvm)
        xc_cpuid_hvm_policy(domid, input, regs, is_pae, is_nestedhvm);
    else
        xc_cpuid_pv_policy(domid, input, regs, guest_64bit);
  ...
  )
{
    ...
    {
        cpuid(input, regs);
        xc_cpuid_policy(domid, input, regs, is_hvm, guest_64bit, is_pae, is_nestedhvm);

        if ( regs[0] || regs[1] || regs[2] || regs[3] )
        {
            rc = set_cpuid(domid, input, regs);
            if ( rc )
                return rc;
        }
